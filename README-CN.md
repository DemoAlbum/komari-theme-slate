...
| 分组 | 配置 |
| --- | --- |
| 基础设置 | 默认外观、默认视图、默认语言、节点默认排序及方向 |
| 首页 | 顶部统计卡片、地图视图 |
| 节点列表 | 节点标签、账单信息、资源总量、网络流量、Swap、运行时间及刷新间隔 |
| 表格列 | 名称、状态、系统、运行时间、CPU、内存、磁盘、网速、总流量的独立显示开关（默认全部开启） |
| 网络质量 | 表格与卡片可独立显示线路延迟及丢包；展示前三条监测线路和最近一小时内可用采样的 10 段色条，每分钟刷新，悬停可查看数值 |
| 游客显示 | 价格、到期状态 |

访客选择外观、语言、视图或节点排序后会写入本地存储（`appearance`、`language`、`slate:view`、`slate:sort`、`slate:sort-direction`），再次打开页面时优先使用本地值；没有本地排序偏好时，节点会直接按后台默认规则显示。游客价格和到期状态默认关闭；登录用户不受这两项限制。关闭地图后，已保存的地图视图会自动回退到可用视图。
